...
/*    var imageMaterial = new THREE.ShaderMaterial( 
    ...
    var imageGeometry = new THREE.PlaneGeometry( -150, 75, 10, 10 );
    ...
    var userGeometry = new THREE.SphereGeometry( 2, 16, 16 );
    var userMesh = new THREE.Mesh( userGeometry, userMaterial );
    userMesh.name = "user";
    userMesh.position.z = 350.0;
    userMesh.geometry.computeBoundingSphere();
...
function LoadData()
{
    ...
    {
        urls: [ 'data/audio/mainLoop_v1.m4a' ]
        , autoplay: false
        , loop: true
        , volume: 1.0
        , onload: function() 
    ...
    {
        urls: [ 'data/audio/animation1_v1.m4a' ]
        , autoplay: false
        , loop: true
        , volume: 1.0
        , onload: function() 
    ...
    {
        urls: [ 'data/audio/animation2_v1.m4a' ]
        , autoplay: false
        , loop: true
        , volume: 1.0
        , onload: function() 
    ...
    {
        urls: [ 'data/audio/animation3_v1.m4a' ]
        , autoplay: false
        , loop: true
        , volume: 1.0
        , onload: function() 
...
//***/
    ...
    scene000.meshLineMaterial = new THREE.LineBasicMaterial( { color: 0x75787B, opacity: 1.0 } );
...
function update(dt)
{
    ...
    //
    if( PX.AnimSceneArray[ PX.currentAnimIndex ].name !== "SHBADGE" )
    ...
    if( PX.AnimSceneArray[ PX.currentAnimIndex ].name === "SH000" )
    {
        ...
        if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 0 )
        {
            ...
            PX.AssetsDatabase.LoopA.fadeIn( 1.0, 500.0 );
        }
        ...
        if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 5 )
        {
            ...
            PX.currentAnimName = "SH020_060B"; //"SH010_RtoL";
            PX.currentAnimIndex = FindSceneIndexByName( PX.currentAnimName );
            PX.AnimSceneArray[ PX.currentAnimIndex ].AddTo( scene );
            ...
            PX.AssetsDatabase.LoopA.fadeOut( 0.0, 500.0, function ()
            {
                console.log( "PX.AssetsDatabase.LoopA.stop" );
                PX.AssetsDatabase.LoopA.stop();
            });
            console.log( "PX.AssetsDatabase.LoopB.play" );
            PX.AssetsDatabase.LoopB.fadeIn( 1.0, 500.0 );
        }

    }


    // SH020_060B: Explosion, etc.
    if( PX.currentAnimName === "SH020_060B" )
    {
        ...
        if( PX.AnimSceneArray[ PX.currentAnimIndex ].IsFinished() )
        {
            ...
            PX.AssetsDatabase.LoopB.fadeOut( 0.0, 500.0, function ()
            {
    ...
    if( PX.AnimSceneArray[ PX.currentAnimIndex ].name === "SH060B_070B" )
    {
        ...
        {
            PX.AnimSceneArray[ PX.currentAnimIndex ].AnimateWithRotation( PX.AnimSceneArray[ PX.currentAnimIndex ].pauseStartTime, effect );
        }
        ...
        if( PX.AnimSceneArray[ PX.currentAnimIndex ].IsFinished() )
        {
            ...
            PX.AssetsDatabase.LoopC.fadeIn( 1.0, 500.0 );
        }
    }
    ...
    if( PX.AnimSceneArray[ PX.currentAnimIndex ].name === "SH080D_PRE" )
    {
        ...
        if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 0 )
        {
            ...
            if( PX.AnimSceneArray[ PX.currentAnimIndex ].IsFinished() )
            {
                ...
                PX.AssetsDatabase.LoopC.fadeOut( 0.0, 500.0, function()
                {
                    console.log( "PX.AssetsDatabase.LoopC.stop" );
                    PX.AssetsDatabase.LoopC.stop();
                });
                console.log( "PX.AssetsDatabase.LoopA.play" );
                PX.AssetsDatabase.LoopA.play();
            }
        }
        else
        {
            ...
            else if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 2 )
            {
                ...
                {
                    PX.AnimSceneArray[ PX.currentAnimIndex ].autoColorRadius += frameTime * 5;
                }
    ...
    if( PX.AnimSceneArray[ PX.currentAnimIndex ].name === "SH090A_110B" )
    {
        ...
        if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 0 )
        {
            ...
            PX.AssetsDatabase.LoopD.fadeIn( 1.0, 500.0 );
            PX.AnimSceneArray[ PX.currentAnimIndex ].state = 1;
        } 
        ...
        {
            DrawBadgeCanvas();
            //document.getElementById("canvasImg").src = PX.GeneratedImageData;
    ...
    if( PX.AnimSceneArray[ PX.currentAnimIndex ].name === "SHBADGE" )
    {
        var imageObj = PX.AnimSceneArray[ PX.currentAnimIndex ].FindObject( "image" );
        var infoObj = PX.AnimSceneArray[ PX.currentAnimIndex ].FindObject( "info" );
        var userObj = PX.AnimSceneArray[ PX.currentAnimIndex ].FindObject( "user" );
        var replayObj = PX.AnimSceneArray[ PX.currentAnimIndex ].FindObject( "replay" );
        var shareObj = PX.AnimSceneArray[ PX.currentAnimIndex ].FindObject( "share" );
        var userObjDepthDistance = 350.0;
        ...
        else if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 2 )
        {
            ...
            {
                PX.AnimSceneArray[ PX.currentAnimIndex ].startMeshIndex = 0; // left one ( replay )
                PX.AnimSceneArray[ PX.currentAnimIndex ].startMeshScaleStartTime = currentTime;
                PX.AnimSceneArray[ PX.currentAnimIndex ].state++;
                enableUserScale = true;
            }
            ...
            {
                PX.AnimSceneArray[ PX.currentAnimIndex ].startMeshIndex = 1; // right one ( share )
                PX.AnimSceneArray[ PX.currentAnimIndex ].startMeshScaleStartTime = currentTime;
                PX.AnimSceneArray[ PX.currentAnimIndex ].state++;
                enableUserScale = true;
            }
        ...
        else if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 3 )
        {
            ...
            var ss = 0.5;
            var lenSq = 999;
            if( PX.AnimSceneArray[ PX.currentAnimIndex ].startMeshIndex === 0 )
            {
        ...
        else if( PX.AnimSceneArray[ PX.currentAnimIndex ].state === 5 )
        {
            if( PX.AnimSceneArray[ PX.currentAnimIndex ].startMeshIndex === 0 )
            {
            }
