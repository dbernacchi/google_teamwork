...
function BuildSH000Scene( scene )
{
    ...
    var startMaterial = new THREE.ShaderMaterial( 
    {
		uniforms:
		{
		    DiffuseMap: { type: "t", value: startTexture }
...
function BuildBadgeScene( scene )
{
    ...
    //
    var removeTexture = PX.AssetsDatabase.removeCardboardTexture;
    removeTexture.wrapS = THREE.ClampToEdgeWrapping;
    removeTexture.wrapT = THREE.ClampToEdgeWrapping;
    removeTexture.magFilter = THREE.LinearFilter;
    removeTexture.minFilter = THREE.LinearMipMapLinearFilter;
    removeTexture.anisotropy = renderer.getMaxAnisotropy();
    ...
    var replayMaterial = new THREE.ShaderMaterial( 
    {
		uniforms:
		{
		    DiffuseMap: { type: "t", value: replayTexture }
            , Alpha: { type: "f", value: 1.0 }
        }
		, vertexShader: PX.Shaders.GradSphereVertex
		, fragmentShader: PX.Shaders.DefaultTextureUnPremultiplyFragment
    ...
    var shareMaterial = new THREE.ShaderMaterial( 
    {
		uniforms:
		{
		    DiffuseMap: { type: "t", value: shareTexture }
            , Alpha: { type: "f", value: 1.0 }
		}
		, vertexShader: PX.Shaders.GradSphereVertex
		, fragmentShader: PX.Shaders.DefaultTextureUnPremultiplyFragment
    ...
    var infoMaterial = new THREE.ShaderMaterial( 
    {
		uniforms:
		{
		    DiffuseMap: { type: "t", value: infoTexture }
            , Alpha: { type: "f", value: 1.0 }
        }
		, vertexShader: PX.Shaders.GradSphereVertex
		, fragmentShader: PX.Shaders.DefaultTextureUnPremultiplyFragment
    ...
    var removeMaterial = new THREE.ShaderMaterial( 
    {
		uniforms: 
        {
            DiffuseMap: { type: "t", value: removeTexture } 
            , Alpha: { type: "f", value: 1.0 }
        }
		, vertexShader: PX.Shaders.GradSphereVertex
		, fragmentShader: PX.Shaders.DefaultTextureUnPremultiplyFragment
    ...
    var removeGeometry = new THREE.PlaneGeometry( -150, 150*(64.0 / 1024.0), 10, 10 );

    var removeMesh = new THREE.Mesh( removeGeometry, removeMaterial );
    removeMesh.name = "remove";
    removeMesh.frustumCulled = false;
    removeMesh.position.x = 100000;
    removeMesh.position.y = -50;
    removeMesh.position.z = 350.0;
    removeMesh.scale.x = 0.001;
    removeMesh.scale.y = 0.001;
    removeMesh.scale.z = 0.001;
    shBadgescene.add( removeMesh );
...
function LoadData()
{
    ...
    $.when(
        ...
        , LoadTexture( "removeCardboardTexture", "data/textures/removephone.png" )
    ).done(function ()
    {
        LoadDataNext();
    } );
...
function init()
{
    ...
    gradientShader = new THREE.ShaderMaterial(
    {
		...
		vertexShader: PX.Shaders.GradSphereVertex
		, fragmentShader: PX.Shaders.GradSphereFragment
    });
	...
	gradientViewShader = new THREE.ShaderMaterial( 
    {
		uniforms:
		{
		    DiffuseMap: { type: "t", value: gradientRT }
            , Alpha: { type: "f", value: 1.0 }
		}
		, vertexShader: PX.Shaders.GradSphereVertex
		, fragmentShader: PX.Shaders.DefaultTextureFragment
	} );
    ...
    else
    {
        ...
        else if( currentScene.state === 2 )
        {
            ...
            for( var i=0; i<currentScene.sceneObjectsArray.length; i++ )
            {
                ...
                var ddd = 0.3;
                if( ( phy.vel.lengthSq() > ddd*ddd && phy.changed === false )
                    || ( isRippleEffect && phy.changed === false )
                    )
                {
...
function UpdateShotsSHBADGE( currentTimeMillis )
{
    ...
    if( currentScene.state === 0 )
    {
        var removeObj = currentScene.FindObject( "remove" );
        removeObj.position.x = 10000.0;
        removeObj.position.y = 0.0;
        removeObj.position.z = 350.0;
        removeObj.scale.x = 0.001;
        removeObj.scale.y = 0.001;
        removeObj.scale.z = 0.001;
        removeObj.updateMatrix();                
    ...
    else if( currentScene.state === 3 )
    {
        ...
        if( currentScene.startMeshIndex === 0 )
        {
            ...
            lenSq = dir.lengthSq();
            replayObj.position.x += ( userObj.position.x - replayObj.position.x ) * 0.09;
            replayObj.position.y += ( userObj.position.y - replayObj.position.y ) * 0.09;
            replayObj.position.z += ( userObj.position.z - replayObj.position.z ) * 0.09;
        }
        // Share
        else
        {
            ...
            lenSq = dir.lengthSq();
            shareObj.position.x += ( userObj.position.x - shareObj.position.x ) * 0.09;
            shareObj.position.y += ( userObj.position.y - shareObj.position.y ) * 0.09;
            shareObj.position.z += ( userObj.position.z - shareObj.position.z ) * 0.09;
        }
    ...
    else if( currentScene.state === 5 )
    {
        ...
        {
            if( sceneTimeSecs < 5.0 )
            {
                var removeObj = currentScene.FindObject( "remove" );
                removeObj.position.x = 0.0;
                removeObj.position.y = -50.0;
                removeObj.position.z = 350.0;
                removeObj.scale.x = 1.0;
                removeObj.scale.y = 1.0;
                removeObj.scale.z = 1.0;
                removeObj.updateMatrix();                
            }
            else
            {
